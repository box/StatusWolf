...
//      }
    ...
    place: function(){
	  ...
	  if (this.widget.hasClass('menu-left'))
	  {
		offset.left = offset.left - this.widget.width() + 10;
	  }
    ...
    update: function(newDate){
      ...
      if (!dateStr) {
        ...
        var parseAttempt = this.parseDate(dateStr);

        if (!dateStr || !parseAttempt) {
          var tmp = new Date()
...
//                  break;
            ...
            case 'td':
              if (target.is('.day')) {
                ...
                this.set();
                this.notifyChange();
              } else if (target.is('.adjuster')) {
			    var increment = target.text();
				switch(increment) {
					case '1h':
						if (target.is('.add')) {
					...
					case '10m':
						if (target.is('.add')) {
							this._date.setUTCMinutes(this._date.getUTCMinutes() + 10);
						} else {
							this._date.setUTCMinutes(this._date.getUTCMinutes() - 10);
						}
						this.fillTime();
					...
					case '1w':
						if (target.is('.add')) {
							this._date.setUTCDate(this._date.getUTCDate() + 7);
						} else {
							this._date.setUTCDate(this._date.getUTCDate() - 7);
						}
						this.fillDate();
					...
					case '30d':
						if (target.is('.add')) {
							this._date.setUTCDate(this._date.getUTCDate() + 30);
						} else {
							this._date.setUTCDate(this._date.getUTCDate() - 30);
						}
						this.fillDate();
					...
					case 'Now':
						...
						if (tzoffset > 0) {
							this._date.setUTCHours(this._date.getUTCHours() - (tzoffset / 60));
						} else {
							this._date.setUTCHours(this._date.getUTCHours() + (tzoffset / 60));
						}
						this.fillTime();
    ...
    actions: {
	  ...
	  incrementMinutesByTen: function(e) {
		this._date.setUTCMinutes(this._date.getUTCMinutes() + 10);
	  },
	  ...
	  decrementMinutesByTen: function(e) {
		this._date.setUTCMinutes(this._date.getUTCMinutes() - 10)
	  },
    ...
    change: function(e) {
      ...
      if (this._formatPattern.test(val)) {
        ...
        // JQ MODIFICATIONS - COMMENT OUT THREE VALIDATION LINES:
        //this.setValue(this._date.getTime());
    ...
    _attachDatePickerEvents: function() {
	  ...
	  // this handles adjustment picker clicks
	  this.widget.on('click', '.adjustpicker *', $.proxy(this.click, this));
  ...
  function getTemplate(timeIcon, pickDate, pickTime, is12Hours, showSeconds, collapse) {
    if (pickDate && pickTime) {
      return (
        '<div class="datetimepicker-widget dropdown-menu menu-left">' +
          '<ul>' +
			'<li' + (collapse ? ' class="collapse"' : '') + '>' +
			  '<div class="adjustpicker">' +
			     APGlobal.template +
			  '</div>' +
			'</li>' +
			'<li' + (collapse ? ' class="collapse"' : '') + '>' +
	...
	'<div class="adjustpicker-options">' +
	  '<table class="table-condensed no-border">' +
        ...
        '<tr>' +
		  ...
		  '<td class="adjuster">10m</td>' +
		  '<td class="adjuster">1m</td>' +
		  '<td></td>' +
	      '<td class="adjuster add">1m</td>' +
	      '<td class="adjuster add">10m</td>' +
		  '<td class="adjuster add">1h</td>' +
		'</tr>' +
		'<tr>' +
		  '<td class="adjuster">30d</td>' +
		  '<td class="adjuster">1w</td>' +
		  '<td class="adjuster">1d</td>' +
		  '<td></td>' +
		  '<td class="adjuster add">1d</td>' +
		  '<td class="adjuster add">1w</td>' +
		  '<td class="adjuster add">30d</td>' +
		'</tr>' +
	  '</table>' +
	'</div>';
  var DPGlobal = {
